...
std::string StaticAnalysis::getOpenseesCommand() const
{
	...
	command += "recorder Node -file nodeDisp" + processID + ".out -time -nodeRange " + std::to_string(minNodeTag) + " " + std::to_string(maxNodeTag) + " -dof 1 2 3 4 5 6 disp";
	command += "\n";
	...
	command += "recorder Element -file eleForce" + processID + ".out -time -eleRange " + std::to_string(minEleTag) + " " + std::to_string(maxEleTag) + " force";
	command += "\n";
